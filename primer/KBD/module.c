...
static int __init initialization_routine(void) {
	 ...
	 * IRQF_SHIRQ means we're willing to have othe handlers on this IRQ.
	 */
...
irqreturn_t ir_getchar(int irq, void* dev) {
    ...
    char c = inb(0x60);   // data register (read\write)
    char status = inb(0x64);    // command register (read\ write)
...
static int pseudo_device_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd){
        case IOCTL_KBD:
            ...
            ev_press = 0;
            keyboard.key = KBD_key;
            printk("Get char: %c \n", KBD_key);
            copy_to_user((struct ioctl_kbd *)arg, &keyboard, sizeof(struct ioctl_kbd));
            break;
